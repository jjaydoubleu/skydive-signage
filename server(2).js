...
function activateAudio() {
  ...
  try { sessionStorage.setItem('activated', '1'); } catch(e) {}
  tryPlay();
}

function tryPlay() {
  try {
    var lastFp = sessionStorage.getItem('lastFp') || '';
    if (msgFp !== lastFp && msgType !== 'idle' && audioMap[msgType]) {
      sessionStorage.setItem('lastFp', msgFp);
...
// Check if already activated from a previous page load
try {
  if (sessionStorage.getItem('activated') === '1') {
    document.getElementById('activate').style.display = 'none';
    tryPlay();
  }
} catch(e) {}
