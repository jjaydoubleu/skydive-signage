...
function buildPage(msg) {
  ...
  if (type === 'idle') {
    ...
    cameraHtml += '<div style="display:table-cell; padding:0 10px;">'
               ...
               + '</div></div>';
    // 3rd party camera only shown if available
    if (msg.thirdcam) {
      cameraHtml += '<div style="display:table-cell; padding:0 10px;">'
                 + '<div style="border:2px solid #4a9eff; padding:14px 28px; text-align:center;">'
                 + '<div style="font-size:28px; margin-bottom:6px;">&#128247;</div>'
                 + '<div style="font-size:18px; font-weight:bold; color:#4a9eff; letter-spacing:1px;">3RD PARTY CAM</div>'
                 + '<div style="font-size:13px; color:#aaaaaa; margin-top:4px;">Available</div>'
                 + '</div></div>';
    }
    cameraHtml += '</div></div>';
