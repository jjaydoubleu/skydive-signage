...
function getSpeechText(msg) {
  const t = msg.trip || '';
  if (msg.type === 'checkin') {
    const mins = msg.briefingMins || 30;
    return `Attention. The ${t} trip is now open for check in. Safety briefing begins in ${mins} minutes. Please proceed to the counter.`;
  } else if (msg.type === 'briefing') {
    return `Attention. This is a call for the ${t} group. Your safety briefing is about to begin. Please make your way to the briefing room now.`;
  } else if (msg.type === 'finalcheck') {
    return `Attention. Briefing is now complete for the ${t} group. Please proceed to the front desk for your final checks. Let staff know if you are taking the bus or driving yourself.`;
  } else if (msg.type === 'weather') {
    return `Attention. The ${t} trip has been cancelled due to weather. Please speak to our staff at the counter to reschedule or arrange a refund.`;
  }
  return '';
}
...
function buildPage(msg) {
  ...
  const speechText = getSpeechText(msg);
  var t = msg.trip || '';
...
(function() {
  try {
    var fp = '${fingerprint}';
    var lastFp = '';
    try { lastFp = sessionStorage.getItem('lastFp') || ''; } catch(e) {}
    if (fp !== lastFp && '${speechText}') {
      try { sessionStorage.setItem('lastFp', fp); } catch(e) {}
