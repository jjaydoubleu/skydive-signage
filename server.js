...
<head>
<meta charset="UTF-8">
<meta http-equiv="refresh" content="3;url=/display.html?fp=${encodeURIComponent(fingerprint)}">
...
<body>
<div id="activate" onclick="activateAudio()">
...
var activated = params.get('activated') === '1';
var lastFp    = params.get('lastFp') || '';

function buildRefreshUrl(act, lfp) {
  return '/display.html?fp=' + encodeURIComponent(msgFp)
       + '&activated=' + (act ? '1' : '0')
       + '&lastFp='    + encodeURIComponent(lfp);
}

// Update the meta refresh URL to carry state forward
var metaRefresh = document.querySelector('meta[http-equiv="refresh"]');
if (metaRefresh) {
  metaRefresh.setAttribute('content', '3;url=' + buildRefreshUrl(activated, lastFp));
}

function activateAudio() {
  activated = true;
  document.getElementById('activate').style.display = 'none';
  // Update meta refresh to carry activated=1 forward
  if (metaRefresh) {
    metaRefresh.setAttribute('content', '3;url=' + buildRefreshUrl(true, lastFp));
  }
  tryPlay();
}
...
function tryPlay() {
  if (!activated) return;
  if (msgFp !== lastFp && msgType !== 'idle' && audioMap[msgType]) {
    lastFp = msgFp;
    // Update meta refresh with new lastFp so next reload knows not to replay
    if (metaRefresh) {
      metaRefresh.setAttribute('content', '3;url=' + buildRefreshUrl(true, msgFp));
    }
    var a = new Audio(audioMap[msgType]);
