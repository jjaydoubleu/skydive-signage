...
function activateDisplay() {
  // Play a silent audio first to unlock audio context on this gesture
  ...
  // Tell server we're activated
  var xhr2 = new XMLHttpRequest();
  xhr2.open('GET', '/activate', true);
    ...
    if (xhr.readyState === 4 && xhr.status === 200) {
      try {
        ...
        if (state.shouldPlay && audioMap[state.type]) {
          ...
          if (metaRefresh) metaRefresh.setAttribute('content', '999');
          ...
          if (playPromise !== undefined) {
            playPromise.then(function() {
              ...
              if (metaRefresh) metaRefresh.setAttribute('content', '3');
            });
          }
          a.onended = function() {
            if (metaRefresh) metaRefresh.setAttribute('content', '3');
      ...
      } catch(e) { console.log('Playstate error:', e); }
    }
  };
  xhr.send();
}, 500);
